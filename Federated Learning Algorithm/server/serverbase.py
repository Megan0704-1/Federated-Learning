...
class Server:
    ...
    def aggregate_parameters(self):
        ...
        with torch.no_grad():
          total_param = 0
          for g_param in self.model.parameters():
            g_param.mul_(0)

          for user in self.selected_users:
            total_param += user.train_samples

          for i, user in enumerate(self.selected_users):
            for g_param, u_param in zip(self.model.parameters(), user.model.parameters()):
              g_param.add_(u_param.detach().clone(), alpha = user.train_samples/total_param)
    ...
    def select_users(self, round, num_users):
        ...
        # assert num_users > total_users
        client_list=[]
        chosen_clients = random.sample(range(total_users), num_users)
        for i, c in enumerate(self.users):
            if i in chosen_clients:
